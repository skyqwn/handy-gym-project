...
var paintPagination = function paintPagination(TOTAL_PAGE) {
  if (1 < CURRENT_PAGE_CONTAINER) {
    ...
    nextLink.href = "/post?page=" + (CURRENT_PAGE_CONTAINER - 1) * PAGE_CONTAINER_SIZE;
  ...
  for (var i = 1; i <= TOTAL_PAGE; i++) {
    ...
    if (i === TOTAL_PAGE && TOTAL_PAGE > PAGE_CONTAINER_SIZE && CURRENT_PAGE_CONTAINER !== TOTAL_PAGE_CONTAINER) {
      var pageLink = document.createElement("a");
      pageLink.href = "/post?page=" + (CURRENT_PAGE_CONTAINER * PAGE_CONTAINER_SIZE + 1);
...
var init = function init() {
  if (PAGE > TOTAL_PAGE) {
    window.location.href = "/post?page=" + TOTAL_PAGE;
  }
